...
publish_to_s3(){
  for provider in "virtualbox" "aws" "vmware"; do
    promote_box $provider $BOSH_LITE_CANDIDATE_BUILD_NUMBER
  done
}
...
promote_box(){
  ...
  s3cmd --access_key=$BOSH_AWS_ACCESS_KEY_ID --secret_key=$BOSH_AWS_SECRET_ACCESS_KEY mv s3://bosh-lite-ci-pipeline/$bosh_name s3://bosh-lite-build-artifacts/$box_name
}
